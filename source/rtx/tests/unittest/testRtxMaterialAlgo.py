...
class MaterialAlgoTest(usdex.test.TestCase):
    ...
    def testTexturedMaterialDefinition(self):
        ...
        def checkDiffuseTexture(matPrim, tex, color, diffLayer=False):
            ...
            self.assertEqual(computeEffectiveShaderInputValue(diffuseTexShader.GetInput("file")).path, tex)
        ...
        def checkNormalTexture(matPrim, tex):
            ...
            normalTexShader = UsdShade.Shader(stage.GetPrimAtPath(materialPath.AppendChild("NormalTex")))
            ...
            self.assertEqual(computeEffectiveShaderInputValue(normalTexShader.GetInput("file")).path, tex)
        ...
        def checkOrmTexture(matPrim, tex, r, m, diffLayer=False):
            ...
            ormTexShader = UsdShade.Shader(stage.GetPrimAtPath(materialPath.AppendChild("ORMTex")))
            ...
            self.assertEqual(computeEffectiveShaderInputValue(ormTexShader.GetInput("file")).path, tex)
        ...
        def checkRoughnessTexture(matPrim, tex, r, diffLayer=False):
            ...
            if diffLayer:
                ...
                self.assertFalse(matPrim.GetInput("Roughness"))
            roughnessTexShader = UsdShade.Shader(stage.GetPrimAtPath(materialPath.AppendChild("RoughnessTex")))
            ...
            self.assertEqual(computeEffectiveShaderInputValue(roughnessTexShader.GetInput("file")).path, tex)
        ...
        def checkMetallicTexture(matPrim, tex, m, diffLayer=False):
            ...
            if diffLayer:
                ...
                self.assertFalse(matPrim.GetInput("Metallic"))
            metallicTexShader = UsdShade.Shader(stage.GetPrimAtPath(materialPath.AppendChild("MetallicTex")))
            ...
            self.assertEqual(computeEffectiveShaderInputValue(metallicTexShader.GetInput("file")).path, tex)
        ...
        def checkOpacityTexture(matPrim, tex, o, diffLayer=False):
            ...
            opacityTexShader = UsdShade.Shader(stage.GetPrimAtPath(materialPath.AppendChild("OpacityTex")))
            ...
            self.assertEqual(computeEffectiveShaderInputValue(opacityTexShader.GetInput("file")).path, tex)
        ...
        with usdex.test.ScopedTfDiagnosticChecker(self, [(Tf.TF_DIAGNOSTIC_WARNING_TYPE, ".*doesn't exist in the current edit target layer")] * 6):
            ...
            checkRoughnessTexture(material, roughnessTexture, roughness, diffLayer=True)
            checkMetallicTexture(material, metallicTexture, metallic, diffLayer=True)
